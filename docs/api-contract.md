...
克隆 Project 是便捷复用操作，用于基于已有 Project 创建一个可独立修改的新 Project。它不引入来源追踪、版本分支或新的执行语义。
...
- 新 Project 必须生成独立 `projectId`，不得覆盖或修改原 Project。
- 新 Project 复制原 Project 的基本配置、表配置、表级行数、清空策略、执行顺序快照和 Project 内关系配置。
- 字段生成规则仍归属 Schema 层；克隆 Project 只保留规则引用语义，不复制字段规则为 Project 私有副本。
- 新 Project 不继承原 Project 的执行历史，后续执行历史只关联新 Project。
- 不新增 `sourceProjectId`、`cloneFromProjectId`、`derivationType` 等追踪字段；克隆只是服务层语法糖。

### 11.9 从已有 Project 的部分表创建新 Project

子集创建 Project 是便捷复用操作，用于在不破坏原 Project 的前提下删除不需要的表配置，快速得到新的临时数据集 Project。该操作必须保持 Project 执行范围合法，不允许为了子集创建引入新的依赖图、行数规划或关系取值复杂性。

```http
POST /api/projects/{projectId}/subset
```
...
{
  "name": "Demo Data 子集",
  "description": "仅用于订单相关临时数据集",
  "projectTableIds": ["pt_orders", "pt_customers"]
}
```
...
```ts
type CreateProjectSubsetResponseDTO = {
  project: ProjectDetailDTO;
  warnings: Array<{
    code: string;
    tableName?: string;
...
- 新 Project 必须生成独立 `projectId`，不得覆盖或修改原 Project。
- `projectTableIds` 表示用户希望保留的原 Project 表配置；服务层只在原 Project 已有配置范围内过滤，不从数据库重新推导新的表集合。
- 子集表集合必须包含所选表在原 Project 中需要的主表或执行上游表；主表包括 Parent 表和 BaseTable 表。若缺少必要上游表，服务层必须返回阻断错误，前端应要求用户补选或取消操作。
- 不允许用户只保留子表而丢弃原 Project 中存在的必要主表或执行上游表。
- 如果原 Project 本身已采用无主表、外部来源或父表不在 Project 内的关系策略，新 Project 继承原有策略，不新增关系取值策略。
- 新 Project 只保留最终表集合内的 `ProjectTable` 配置，以及两端表都仍存在于最终表集合内的 `ProjectTableRelation` 配置；关联到已删除表的关系配置随表删除而删除。
- 表级行数、清空策略和执行顺序快照按保留表复制；保存时可由后续 Project service 按既有规则重新校验或重算执行顺序。
- 字段生成规则仍归属 Schema 层；子集 Project 只通过保留表继续引用对应字段规则，不复制字段规则为 Project 私有副本。
- 新 Project 不继承原 Project 的执行历史，后续执行历史只关联新 Project。
- 不新增 `sourceProjectId`、`cloneFromProjectId`、`derivationType` 等追踪字段；子集创建只是服务层语法糖。
...
| `ProjectFacade` | `ListProjects`、`CreateProject`、`GetProject`、`UpdateProject`、`UpdateProjectTables`、`ListAvailableTables`、`DeleteProject`、`CloneProject`、`CreateProjectSubset` | Project API |
