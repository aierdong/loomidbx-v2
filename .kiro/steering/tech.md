...
后端 Go 代码必须添加必要注释，保持领域含义、契约边界和生成绑定语义清晰：

- 常量和枚举值必须添加注释，说明业务含义、使用场景或取值约束。
- `struct` 及其中所有字段必须添加注释。字段注释放在字段上方，不放在字段右侧；每组“字段注释 + 字段定义”与下一组之间用空行分开。
- 导出方法必须添加方法注释，说明方法作用、输入参数含义和输出参数含义。涉及错误返回时，应说明主要错误场景或调用方需要关注的失败条件。
